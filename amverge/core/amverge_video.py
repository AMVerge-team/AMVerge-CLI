...
class AmvergeVideo:
    ...
    def keyframes(self) -> list[float]:
        from .keyframe_align import get_keyframe_timestamps_pyav
    ...
    def cut_scene(
        ...
        *,
        ...
        """
        from .smart_cut import cut_scene as _cut
        if use_cuda is None:
            try:
                import torch
    ...
    def copy_segment(
        ...
        *,
        ...
        """
        from .codec_utils import AUDIO_FFMPEG
    ...
    def decode_frames(self) -> "np.ndarray":
        ...
        """
        try:
            from .scene_detection import decode_video_frames_nelux
            return decode_video_frames_nelux(str(self._path))
        except ImportError:
            import numpy as np
            from .transnet_constants import FRAME_HEIGHT, FRAME_WIDTH, FRAME_CHANNELS, FRAME_BYTES
            cmd = [
                self._ffmpeg, "-y",
                "-i", str(self._path),
                "-pix_fmt", "rgb24",
